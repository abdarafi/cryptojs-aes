...
username = CryptoJS.AES.encrypt(JSON.stringify('klinik_healthkathonPro'), key, { format: CryptoJSAesJson }).toString();
...
for(i in array) {
    ...
    data["password"] = CryptoJS.AES.encrypt(JSON.stringify(array[i]), key, { format: CryptoJSAesJson }).toString();
    request.post({url:url, formData: data}, function(err, httpResponse, body) {
        if (err) {
          return console.error('gagal:', err);
        }
      
        console.log('berhasil!  respon:', body);
      });
}
